...
def delete_student(request,id):
        ...
        return JsonResponse({"message":"The method is not allowed"},status=status.HTTP_405_METHOD_NOT_ALLOWED)
@api_view(['GET','POST'])
def get_all_or_add_groups(request):
    if request.method=='GET' :
        ...
        if group.is_valid():
            result=Group.objects.filter(name=group.validated_data['name'])
            if result :
                return JsonResponse({"message" : "The group name already exists"},status=status.HTTP_400_BAD_REQUEST)
            group.save()
            return Response(status=status.HTTP_201_CREATED)
